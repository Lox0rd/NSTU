...
#include <stdlib.h>
#include <string.h>
#include <unistd.h>  // для доступа к функциям POSIX (вместо windows.h)
...
void print_usage() {
    printf("Использование: pack <файл> compress\n");
...
int write_record(FILE* f, Record* r, int offset) {
    fseek(f, offset, SEEK_SET);
...
int main(int argc, char* argv[]) {
    ...
    if (!temp) {
        perror("Не может создать временный файл");
        free(active);
        fclose(file);
        return 1;
    }
    ...
    for (i = 0; i < h.active_count; i++) {
        ...
        if (!write_record(temp, &active[i], offset)) {
    ...
    // Заменяем файл
    if (remove(argv[1]) != 0) {
        perror("Не может удалить исходный файл");
        return 1;
    }

    if (rename(temp_name, argv[1]) != 0) {
        perror("Не может переименовать временный файл");
        return 1;
    }
