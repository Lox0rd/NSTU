...
void printHelp(const char* programName) {
    printf("Использование: %s <имя_файла> <наименование_элемента>\n", programName);
...
int isNameUnique(FILE* file, const Header* header, const char* name) {
    ...
    const int MAX_ITERATIONS = 1000; // Защитный лимит

    while (current != -1) {
        iterationCount++;
        if (iterationCount > MAX_ITERATIONS) {
            printf("Ошибка: обнаружено зацикливание в списке активных элементов\n");
            return 0; // Считаем имя не уникальным при ошибке
        }
